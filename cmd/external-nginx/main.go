...
func main() {
	...
	if len(conf.APIServerHosts) != 0 {
		klog.Info("Multi ApiServer provided .")
		for _, host := range controller.GetHosts(conf.APIServerHosts) {
			kubeClient, err := createApiserverClient(host, conf.RootCAFile, conf.KubeConfigFile)
			if err != nil {
	...
	if len(conf.DefaultService) > 0 {
		err := checkService(conf.DefaultService, kubeClients[0])
		if err != nil {
			klog.Fatal(err)
		}
	...
	if len(conf.PublishService) > 0 {
		err := checkService(conf.PublishService, kubeClients[0])
		if err != nil {
			klog.Fatal(err)
		}
	}
	...
	if conf.Namespace != "" {
		_, err = kubeClients[0].CoreV1().Namespaces().Get(context.TODO(), conf.Namespace, metav1.GetOptions{})
		if err != nil {
	...
	isNetworkingIngressAvailable, k8s.IsIngressV1Beta1Ready, _ = k8s.NetworkingIngressAvailable(kubeClients[0])
	...
	if k8s.IsIngressV1Beta1Ready {
		...
		k8s.IngressClass, err = kubeClients[0].NetworkingV1beta1().IngressClasses().
	...
	conf.Client = kubeClients[0]

	err = k8s.GetIngressPod(kubeClients[0])
	if err != nil {
