...
program runs inside a Kubernetes cluster and local discovery is attempted.`)
		...
		apiserverHosts = flags.String("apiserver-hosts", "",
			`Address of multi Kubernetes API servers.
Takes the form "protocol://address:port,protocol://address:port,protocol://address:port". If not specified, it is assumed the
program runs inside a Kubernetes cluster and local discovery is attempted.`)
